...
def predict_status(ctry,itmtp,aplcn,wth,prdrf,qtlg,cstlg,tknslg,slgplg,itmdt,itmmn,itmyr,deldtdy,deldtmn,deldtyr):
    ...
    with open("D:/GUVI/Integration python with sql/Classification_model.pkl","rb") as f:
...
def predict_selling_price(ctry,sts,itmtp,aplcn,wth,prdrf,qtlg,cstlg,
    ...
    dydy= int(deldtyr)
    #modelfile of the classification
    with open("D:/GUVI/Integration python with sql/Regression_Model.pkl","rb") as r:
        model_regg=pickle.load(r)
...
with st.sidebar:
    option = option_menu('Robert Singh', options=["PREDICT SELLING PRICE", "PREDICT STATUS"])
